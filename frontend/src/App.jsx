...
const App = () => {
  ...
  // Función de validación de modelo real
  const isRealLLMModel = (model) => {
    const invalidTags = [
      'rule_engine', 'structured_tool', 'structured_tool_logic',
      'rag_engine', 'fallback_debug', 'Servidor Offline',
      'Modelo Desconocido', 'Error de Conexión', 'Sincronizando...'
    ];
    return typeof model === 'string' &&
      !invalidTags.includes(model) &&
      (model.includes('gemini') || model.includes('llama') || model.includes('mistral') || model.includes('gemma'));
  };
  ...
  useEffect(() => {
    const syncEngine = async () => {
      try {
        ...
        if (data && data.model && isRealLLMModel(data.model)) {
          setAiModel(data.model);
        } else if (data && data.model) {
          // Si es una etiqueta pero no un modelo real, no actualizamos el aiModel global
          console.log("Detectado tag interno en sync:", data.model);
        } else {
    ...
    try {
      const config = {
        ...
        ...(body && {
          body: JSON.stringify({
            ...body,
            ...(isRealLLMModel(aiModel) ? { model: aiModel } : {})
          })
        })
      };
      ...
      if (data?.model && isRealLLMModel(data.model)) {
        setAiModel(data.model);
      }
      ...
      if (endpoint === 'chat') {
        ...
        setSummaryResult(cleaned);
        setWordCount(cleaned.split(/\s+/).filter(w => w !== '').length);
      } else if (endpoint === 'faq') {
        setFaqResult(cleaned);
        setWordCount(cleaned.split(/\s+/).filter(w => w !== '').length);
      }
      ...
      if (endpoint === 'chat') {
        ...
        // En caso de error no sobreescribimos el resultado previo si existía
  ...
  const getActiveResult = () => {
    if (activeTab === 'summary') return summaryResult;
    if (activeTab === 'faq') return faqResult;
    return '';
  };
    ...
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col font-sans antialiased text-slate-900">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-6 grid grid-cols-1 lg:grid-cols-12 gap-8 mt-4">
            ...
            <nav className="space-y-1">
              {['summary', 'faq', 'chat'].map((tab) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-5 py-4 rounded-[1.8rem] font-bold transition-all ${activeTab === tab
        ...
        <section className="lg:col-span-9">
          <motion.div layout className={`bg-white rounded-[3.5rem] shadow-2xl shadow-slate-300/20 border border-slate-100 p-10 min-h-[650px] flex flex-col relative justify-between"${activeTab === 'chat' && messages.length === 0
              ? 'min-h-[400px]'
              : 'min-h-[650px]'
            }`}
          >
            <div className="mb-10 flex justify-between items-start">
              ...
              {getActiveResult() && <div className="bg-slate-50 px-4 py-2 rounded-xl text-[10px] font-black text-slate-600 border border-slate-200">{wordCount} PALABRAS</div>}
            </div>
                    ...
                    <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                      {messages.map((msg, i) => (
                        <div key={i} className={msg.role === 'user' ? 'text-right' : 'text-left'}>
                          <div className={`inline-block px-4 py-3 rounded-2xl max-w-[70%] ${msg.role === 'user'
                    ...
                    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100 text-lg leading-relaxed">
                      ...
                      {activeTab === 'faq' ? (
                        <div className="space-y-8">
                          {parseFAQ(getActiveResult()).map((item, i) => (
